...
class FormProcessor
{
    ...
    private function _checkRequiredFields($form)
    {
        ...
        foreach ($form['fields'] as $field => $properties) {
            ...
            $fname = preg_replace('/\[\]$/', '', $field);
            $isFile = array_key_exists($fname, $_FILES);
            if (!$isFile && (!array_key_exists($field, $_REQUEST) || ($_REQUEST[$field] !== "0" && empty($_REQUEST[$field])))) {
    ...
    private function _formSubmission($form)
    {
        ...
        if (!$emailFrom) {
            $emailFrom = (array_key_exists('email', $_REQUEST) && !empty($_REQUEST['email'])) ? $this->_cleanupEmail($_REQUEST['email']) : '';
        }

        $to = $form['email']['to'];
        $subject = $form['subject'];

        $formFilesData = $this->_getFormFilesData();
        $formFiles = $formFilesData['files'];

        $message = $this->_getEmailBody($subject, $form, $formFiles);

        $mail = new FormMail();
        foreach ($formFiles as $file) {
            $mail->addAttachment($file['tmp_name'], $file['name']);
        }
        //if you change next line, you need change test regexp (search by next line)
        $sent = $mail->sendMail($emailFrom, $to, $subject, $message, true, $emailFrom);
    ...
    private function _getEmailBody($subject, $form, $files = array()) {
        $emailMsg = $form['email_message'];
        $fields = $form['fields'];
...
STYLES;
        ...
        foreach ($files as $j => $file) {
            array_push($sortedFields, array(
                'field' => 'file',
                'properties' => array(
                    'type' => 'file',
                    'value' => $file['name'],
                    'label' => ucfirst('File-' . ($j + 1))
                ),
            ));
        }
        ...
        foreach ($sortedFields as $fieldWrapper) {
            ...
            $message .= '<strong>' . $this->_encodeValue(self::_decodeASCII($fieldWrapper['properties']['label'])) . ':</strong>';
    ...
    private function _getFormFilesData() {
        $formFiles = $_FILES;
        $errors = array();
        ...
        if (isset($formFiles['file'])) {
            ...
            foreach ($formFiles['file']['name'] as $k => $v) {
                if ($v) {
                    ...
                    array_push($fileListToEmail, array(
                        'name' => $v,
    ...
    private function _decodeASCII($text)
    {
...
/**
 * Wrapper for PhpEmail Class.
 */
class FormMail extends PHPMailer
{
    ...
    public function sendMail($from, $recipient, $subject, $body, $mode = false, $replyTo = null, $attachment = null)
    {
        ...
        if (parent::addAddress($recipient) === false)
        {
            return false;
        }
